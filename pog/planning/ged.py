...
def ged(g1, g2, with_path=False):
    if with_path:
        for node_edit_path, edge_edit_path, cost in nx.algorithms.similarity.optimize_edit_paths(g1.graph, g2.graph, roots = (g1.root, g2.root), node_match=node_match, edge_match=edge_match):
            if check_node_distance(g1, g2, node_edit_path):
                return cost, ((node_edit_path, edge_edit_path))
    else: # Faster
        cost = nx.algorithms.similarity.graph_edit_distance(g1.graph, g2.graph, node_match=node_match, edge_match=edge_match, roots=(g1.root, g2.root))
        return cost
...
def ged_seq(init : Graph, goal : Graph):
    ...
    for edge_pair in edit_path[1]:
        if edge_pair[0]:
            if init.edge_dict[edge_pair[0][1]].relations[AffordanceType.Support]['dof'] == 'fixed': 
                continue
        if edge_pair[0] == None:
    ...
    edge_edit_pairs = optimal_edge_edit_pair(init, goal, edge_delete_pairs, edge_add_pairs)            
