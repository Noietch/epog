...
from pog.planning.action import (
    Action,
    ActionConstraints,
    ActionType,
    action_seq_generator,
    updateGraph,
)
from pog.planning.searchNode import SearchNode
from pog.planning.searchProblem import Path, Search_problem
...
class PlanningOnGraphPath(Path):
    ...
    def __repr__(self):
        ...
        if self.arc is None:
            ...
            return (
                str(self.initial) + " ==> " + str(self.arc.action)
            )  # + str(self.arc.to_node.constraints)
...
class PlanningOnGraphProblem(Search_problem):
    ...
    def insert_action_unstable_or_collision(
        self,
        node: SearchNode,
        current: Graph,
        new_action: Action,
        ...
        if new_action.add_edge[1] in unstable_nodes:
            ...
            for succ in current.graph.successors(new_action.add_edge[1]):
                del_succ = Action(
                    ((new_action.add_edge[1], succ), None), action_type=ActionType.Pick
                )
                if del_succ in new_action_seq:
                    ...
                    for action in new_action_seq:
                        if (
                            ...
                            break
                    to_ground = Action(
                        ((new_action.add_edge[1], succ), (self.parking_place, succ)),
                    ...
                    from_ground = Action(
                        ((self.parking_place, succ), (new_action.add_edge[1], succ)),
                        action_type=ActionType.PicknPlace,
                        optimized=False,
                    )
                    constraints.addConstraint(to_ground, temp_new_action_pick)
    ...
    def action_is_place(self, action: Action):
        return (
            action.action_type == ActionType.Place
            or action.action_type == ActionType.PicknPlace
        )
        ...
        for neighbor in node.selectAction():
            ...
            if self.action_is_place(new_action):
                current, new_action, new_action_seq, constraints, moving_cost = (
                    self.insert_action_unstable_or_collision(
                        node, current, new_action, new_action_seq, constraints
                    )
                )
                if current is None:
                    continue
