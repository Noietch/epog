...
class Action:
    ...
    def __repr__(self) -> str:
        if self.action_type == ActionType.Pick:
            return f"Pick {self.del_edge[1]} from {self.del_edge[0]}"
        elif self.action_type == ActionType.Place:
            return f"Place {self.add_edge[1]} on {self.add_edge[0]}"
        elif self.action_type == ActionType.Open:
            return f"Open {self.del_edge[1]}"
        elif self.action_type == ActionType.Close:
            return f"Close {self.del_edge[1]}"
        elif self.action_type == ActionType.PicknPlace:
            return f"PicknPlace {self.del_edge[1]} to {self.add_edge[0]}"
        elif self.action_type == ActionType.Walk:
            return f"Walk {self.del_edge[1]}"
        else:
            raise NotImplementedError(f"self.action_type = {self.action_type}")
...
def updateGraph(current: Graph, goal: Graph, action_seq: list[Action], optimize=False):
    ...
    for action in action_seq:
        if action is None:
            ...
            # move to the node
            moving_cost += current.get_distance_btw_nodes(
            ...
            logging.error(f"Unknown action type: {action.action_type}")
    return moving_cost
