...
from pog.algorithm.params import SAVE_HISTORY, STEP_SIZE, MAX_ITER, MAX_TEMP, TEMP_COEFF, \
...
if __name__ == "__main__":
    ...
    object_pose_dict, bounds = gen_bound(pose)
    start = time.time()
    best, best_eval = simulated_annealing([objective],
                                          sg,
                                          random_start=True,
                                          verbose=True)
    end = time.time()
    print('Run {} iterations in {:.4f} seconds'.format(MAX_ITER, end - start))
