...
def objective_history(arr, object_pose_dict, sg):
    ...
    total_weight = (
        WEIGHT_SINGLE_STABILITY + WEIGHT_COLLISION + WEIGHT_TOTAL_STABILITY,
    )
    return cost_2 + cost_3, (
        step_direction_2 * WEIGHT_SINGLE_STABILITY + step_direction_3 * WEIGHT_COLLISION
    ) / total_weight
...
def objective_parent_child_stability(arr, object_pose_dict, sg: Graph):
    ...
    for obj in object_pose_dict:
        step_direction[object_pose_dict[obj][0] : object_pose_dict[obj][0] + 2] = (
            -np.array(temp_com[0:2]) / max(EPS, np.linalg.norm(temp_com[0:2]))
        )
...
def objective_single_stability(arr, object_pose_dict, sg: Graph):
    ...
    for key, value in pose.items():
        ...
        ) / max(EPS, np.linalg.norm(temp_com[0:2]))
...
def constraint_collision(arr, object_pose_dict, sg: Graph):
    ...
    if is_collision:
        for name in names:
            logging.debug(f"Object: {name[0]} and {name[1]} are in collision.")
...
def objective_collision(arr, object_pose_dict, sg: Graph):
    ...
    for contact_data in contacts:
        ...
        if (
            collision_pair[0] in max_penetration_dict
            and contact_data.depth > MARGIN_COLLISION
            and (
                collision_pair[1] not in max_penetration_dict[collision_pair[0]]
        ...
        if (
            collision_pair[1] in max_penetration_dict
            and contact_data.depth > MARGIN_COLLISION
            and (
                collision_pair[0] not in max_penetration_dict[collision_pair[1]]
    ...
    for obj1, pair_dict in max_penetration_dict.items():
        ...
        for obj2, dist in pair_dict.items():
            if obj1 in object_pose_dict and obj2 in object_pose_dict:
                ...
                step_direction[
                    ...
                    / total_dist
                    * direction21
                    / max(EPS, np.linalg.norm(direction21))
                )
...
def pose2arr(pose, object_pose_dict, arr=None):
    if arr is None:
        arr = []
