...
from pog.planning.action import Action, ActionConstraints, ged_seq, ActionType
from epog.utils.gpt_helper import ChatGPT
from epog.algorithm.epog.fake_simulator import MotionError, MotionErrorType
from epog.algorithm.llm_prompt import get_most_likely_room, \
...
class Planner:
    ...
    def get_most_likely_receptacle(self, object_category: str, room: str, receptcle: list[str]) -> str:
        if self.global_expert:
    ...
    def rule_based_replanner(self, message: MotionError) -> list[Action]:
        if message.error_type == MotionErrorType.AccessError:
            ...
            for node_id in message.involved_nodes_id:
                open_action = Action(((node_id, node_id), (node_id, node_id)), action_type=ActionType.Open)
                insert_actions.append(open_action)
            insert_actions.append(message.failure_action)
            for node_id in message.involved_nodes_id:
                close_action = Action(((node_id, node_id), (node_id, node_id)), action_type=ActionType.Close)
            ...
            for node_id in message.involved_nodes_id:
                insert_actions.append(Action(((parent_id, node_id), None), 
                                                action_type=ActionType.Pick))
                insert_actions.append(Action((None, (message.parking_place, node_id)), 
            ...
            parent_id, child_id = message.failure_action.del_edge
            for node_id in message.involved_nodes_id:
                insert_actions.append(Action(((child_id, node_id), None), 
                                             action_type=ActionType.Pick))
                insert_actions.append(Action((None, (message.parking_place, node_id)), 
                                                action_type=ActionType.Place))
            insert_actions.append(message.failure_action) 
