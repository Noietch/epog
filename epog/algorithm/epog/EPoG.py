...
from epog.envs.graph import SceneNode, SceneEdge, SceneGraph
from epog.envs.proc_env import ProcEnv, Observation, update_graph, print_action
...
class PlanningMetrics:
    ...
    def __add__(self, other):
        return PlanningMetrics(self.moving_cost + other.moving_cost, self.expand_node + other.expand_node, self.action_seq_len + other.action_seq_len)
    
    def __truediv__(self, other: int):
        return PlanningMetrics(self.moving_cost / other, self.expand_node / other, self.action_seq_len / other)
...
class EPoG:
    def __init__(self, 
        ...
        self.env = env
        self.belif_graph = env.get_init_graph()
        self.task_graph = env.task_graph
        self.path_opt = path_opt
        self.agent = Planner(global_agent=global_agent,
    ...
    def get_lost_nodes(self) -> list[int]:
        node_ids = []
        for node_id in self.task_graph.node_dict.keys():
            if node_id not in self.belif_graph.node_dict:
                node_ids.append(node_id)
        for node_id in self.belif_graph.node_dict.keys():
            assert node_id in self.task_graph.node_dict or \
                self.belif_graph.node_dict[node_id].is_virtual()
        return node_ids

    def init_visited_map(self) -> tuple[dict, dict]:
        # add room nodes to the belif graph
        self.room_visited, self.receptacle_visited = {}, {}
        for room_node in self.belif_graph.room_nodes:
            self.room_visited[room_node.id] = False
            self.receptacle_visited[room_node.id] = {}
            for node in self.belif_graph.receptacle_nodes[room_node.id]:
    ...
    def get_unvisited_rooms_type(self) :
        ...
        for room_id, visited in self.room_visited.items():
            if not self.check_room_visited(room_id):
                room_node = self.belif_graph.node_dict[room_id]
    ...
    def get_unvisited_receptacles_type(self, room_id: int) -> set:
        ...
        for receptacle_id in self.receptacle_visited[room_id]:
            if not self.receptacle_visited[room_id][receptacle_id]:
                receptacle_node = self.belif_graph.node_dict[receptacle_id]
    ...
    def insert_to_room(self, lost_node: SceneNode) -> tuple[SceneNode, float]:
        ...
        room_type_result = self.agent.get_most_likely_room(node_category, unvisited_rooms)
        room_type = self.env.get_most_similar_room(room_type_result)
        rooms = self.belif_graph.get_room_by_type(room_type)
        index = random.randint(0, len(rooms)-1)
        return rooms[index]  # TODO: if muti-room setting is necessary

    def insert_to_receptacle(self, lost_node: SceneNode, room_node: SceneNode) -> SceneNode:
        node_category = lost_node.category
        unvisited_receptacles = self.get_unvisited_receptacles_type(room_node.id)
        assert len(unvisited_receptacles) > 0
        result = self.agent.get_most_likely_receptacle(node_category, room_node.room_type, unvisited_receptacles)
        receptacle_type_result = result
        receptacle_type = self.env.get_most_similar_receptacle(room_node.id, receptacle_type_result)
        receptacles = self.belif_graph.get_receptacle_by_type(room_node.id, receptacle_type)
    ...
    def insert_per_lost_node(self, lost_node_id: int):
        ...
        self.belif_graph.add_node(lost_node)
        new_edge = SceneEdge(receptacle_node.id, lost_node.id, 
                  semantic_info= {'relationType': 'on'})
        new_edge.add_relation(receptacle_node, lost_node)
        self.belif_graph.add_edge(new_edge)
        print(f"[Estimate] insert {lost_node} to {receptacle_node} in {room_node}")
    ...
    def insert_exploration_action(self, rough_plan_step: Action):
        robot_location_node = self.belif_graph.robot_node_id
        if rough_plan_step.action_type == ActionType.Pick:
            ...
            receptacle_node = self.belif_graph.get_parent_receptacle(child_id)
            if receptacle_node.id != robot_location_node:
                insert_explore_action = Action(action_type=ActionType.Walk, 
            ...
            if child_id in set(self.belif_graph.robot.nodes) and \
                parent_id in set(self.belif_graph.robot.nodes):
                print(f"Skip: {child_id} has been placed in {parent_id}")
                return True, None
            receptacle_node = self.belif_graph.get_parent_receptacle(parent_id)
            if receptacle_node.id != robot_location_node:
                insert_explore_action = Action(action_type=ActionType.Walk, 
                                               edge_edit_pair=((receptacle_node.id, receptacle_node.id), None))
                return False, insert_explore_action
    ...
    def add_new_receptacle_node(self, node: SceneNode, edge: SceneEdge):
        ...
        print(f"add new receptacle node {node}")

    def update_belif_graph(self, obs: Observation,
                           action: Action) -> bool:
        ...
        replan_flag = False
        belif_nodes_id = set(self.belif_graph.graph.nodes)
        # in belif graph 
        for node_id in belif_nodes_id:
            assert node_id in self.task_graph.node_dict.keys() or \
                self.belif_graph.node_dict[node_id].is_virtual()
            belif_node = self.belif_graph.node_dict[node_id]
            if node_id not in self.belif_graph.edge_dict.keys():
                continue
            belif_edge = self.belif_graph.edge_dict[node_id]
            if node_id in obs.node_visible.keys():
                if not belif_node.is_truth:
                    ...
                    if obs_edge == belif_edge:
                        ...
                        print(f"update belif node {obs_node}")
                    else:
                        # update the node relation
                        self.belif_graph.update_egde(belif_edge, obs_edge)
                        self.belif_graph.update_node(belif_node, obs_node)
                        replan_flag = True
                        belif_node.set_to_truth()
                        print(f"update belif graph {obs_node}: {belif_edge} -> {obs_edge}")
                        if obs_edge.parent_id not in self.belif_graph.node_dict.keys():
                ...
                bnode_parent_id = self.belif_graph.get_parent_receptacle(node_id).id
                if bnode_parent_id in self.receptacle_visited[robot_room_id] and \
                    self.receptacle_visited[robot_room_id][bnode_parent_id]:
                    # re insert the node
                    print(f"{belif_node} is not visible in {obs.room_node}")
                    self.belif_graph.remove_node(node_id)
                    self.insert_per_lost_node(node_id)
                    replan_flag = True
        return replan_flag

    def global_plan(self) -> list[Action]:
        action_seq, constraints = self.agent.generate_constraint_set(self.belif_graph, self.task_graph)
        problem = PlanningOnDynamicGraphProblem(self.belif_graph, 
                                                self.task_graph, 
                                                env=self.env,
                                                action_seq=action_seq,
                                                constraints=constraints)
        min_cost_path, _ = test(Searcher, problem=problem, pruned=True, find_min_path=self.path_opt)
        print("Global Planning")
        return path_to_action_sequence(min_cost_path)[1:]

    def local_plan(self, current_graph: SceneGraph, action_sequence:list[Action]) -> None:
        for action in action_sequence:
            exception, message = self.simulator.simulate_step(graph=current_graph,
                                                              action=action)
            if exception:
                print(f"exception: {message}")
                message.parking_place = self.belif_graph.get_current_room_id()
                insert_actions = self.agent.generate_reslove_actions(message=message)
                print(f"reslove plan: {insert_actions}")
    ...
    def roll_out(self, action: Action) -> list[Action]:
        ...
        replan_flag = self.update_belif_graph(obs, action)
        if not is_skip:
    ...
    def eval(self, visualize: bool=False) -> PlanningMetrics:
        moving_cost = 0
        sx, sy, sz = self.belif_graph.node_dict[self.env.respawn_node_id].position
        ...
        for action in self.traj:
            if action.action_type == ActionType.Walk:
                ...
                node = self.belif_graph.node_dict[node_id]
                x, y, z = node.position
                real_path.append((x, z))
                moving_cost += self.env.nav_map.distance(real_path[index], real_path[index+1])
                index += 1
        ...
        for action in self.traj:
            print_action(action, self.belif_graph)
        result = PlanningMetrics(moving_cost, node_expanded, len(self.traj), self.is_goal_achieved())
        print('Moving Cost:', moving_cost)
        return result

    def is_goal_achieved(self) -> bool:
        _, _, _, edge_edit_pairs = ged_seq(self.belif_graph, self.task_graph)
