...
class LLM(EFS):
    ...
    def get_plan(self, current_location, current_description, goal_description) -> None:
        ...
        system_prompt = f'''You are an AI robot that generate a plan of actions to reach the goal.
                            ...
                            x, y should be the id of the object or location. For the same object, Pick must be performed before Place.
                        ''' 
    ...
    def main_loop(self):
        ...
        while len(rough_plan) > 0 and co < 20:
            ...
            if not is_success:
                ...
                print("this action is passing check1:", rough_plan_step)
                success = self.roll_out(rough_plan_step)
                if not success:
                    print("this action is not passing check2:", rough_plan_step)
                    current_description = self.output_belief_graph() 
                    goal_description = self.output_task_graph()
                    current_location = self.belif_graph.robot_node_id
