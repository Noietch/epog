...
class NavigationMap:
    def __init__(self, controller: Controller, robot_radius=0.25, grid_size=0.25) -> None:
    ...
    def init_grid_map(self):
        ...
        x_range, z_range = self.convert_to_grid(max(self.reachable_xs) + self.grid_size, 
    ...
    def find_nearest_node(self, x, y):
        x_l = []
        y_l = []
        for ix, iy in zip(self.reachable_xs, self.reachable_zs):
            if x == ix and y == iy:
                continue
            x_l.append(ix)
            y_l.append(iy)
        dlist = [(x - ix) ** 2 + (y - iy) ** 2 for (ix, iy) in zip(x_l, y_l)]
        # except x, y
    ...
    def distance(self, start, goal, show=False) -> float:
        ...
        for i in range(len(full_path) - 1):
            moving_cost += np.sqrt((full_path[i+1][0] - full_path[i][0])**2 + 
