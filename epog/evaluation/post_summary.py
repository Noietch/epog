...
def diff(base: pd.DataFrame, df: pd.DataFrame, save_path: str) -> pd.DataFrame:
    ...
    for task in list(task_list.keys()):
        ...
        for item_df in task_df.itertuples():
            if item_df.success:
                ...
                expand_node_diff_percentage.append((item_df.expand_node - base_item.expand_node) / base_item.expand_node * 100) 
                travel_dist_diff_percentage.append((item_df.travel_dist - base_item.travel_dist) / base_item.travel_dist * 100) 
    ...
    data['expand_node_diff_percentage'].append(np.mean(data['expand_node_diff_percentage']))
    data['travel_dist_diff_percentage'].append(np.mean(data['travel_dist_diff_percentage']))
